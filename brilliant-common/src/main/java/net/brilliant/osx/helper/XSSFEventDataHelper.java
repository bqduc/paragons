...
public class XSSFEventDataHelper {
	...
	public static XSSFEventDataHelper instance(Map<?, ?> params) throws CommonException {
		if (!params.containsKey(OSXConstants.INPUT_STREAM))
			throw new CommonException("No input stream parameter!");
		...
		try {
			if (params.containsKey(OSXConstants.ENCRYPTION_KEYS)) {
				...
				try {
				    ...
				    throw new CommonException("Unable to process encrypted document", ex);
				}
	...
	private OsxBucketContainer extractXlsxData() throws CommonException {
	...
	public OsxBucketContainer parseXlsxData() throws CommonException {
	...
	public DataWorkbook readXlsx() throws CommonException {
		InputStream inputStream = null;
		String sheetName = null;
		ReadOnlySharedStringsTable sharedStringTable = null;
		XSSFReader xssfReader = null;
